...
в него. Роутинг динамический: nginx резолвит `bsl-<proj>` через Docker DNS
(`resolver 127.0.0.11`) и проксирует на `bsl-<proj>:8051/mcp` — **добавлять
2-й, 3-й, n-й проект можно без правки nginx-конфига**, достаточно сервиса
`bsl-<proj>` в `docker-compose.yml`. Условия: бэкенд слушает порт `8051`; имя
проекта — `[a-zA-Z0-9_-]+` (дефис допустим); токен один на развёртывание
(`BSL_MCP_TOKEN`), отдельный токен на проект не предусмотрен. Полный конфиг nginx
и пошаговое добавление проекта — в
[QUICKSTART.md, «Добавить n-й проект»](QUICKSTART.md#4-как-устроены-сервисы-и-url).
